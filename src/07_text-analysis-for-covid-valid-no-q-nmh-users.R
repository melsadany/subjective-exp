...
files2 <- data.frame(author = list.files(path = reddit.dir, pattern = "nmh_user")) %>%
  mutate(file = paste0(reddit.dir,"/", author)) %>%
  mutate(author = sub("\\.rds", "", sub("nmh_user-", "", author)))
f <- files2$file[i]
...
if (nrow(df2)>1) {
  write_rds(df2, file = paste0(reddit.dir, "/text-analyzed/nmh_user-", files2$author[i], ".rds"), 
            compress = "gz")
}
